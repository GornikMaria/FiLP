...
-- 1.1 Реализация
readInt :: IO Int    -- IO (ввод/вывод)
readInt = do
    input <- getLine
    return ( read input :: Int ) -- преобразовать строку в число и вернуть это число как результат с типом IO Int

generateList :: Int -> Int -> Int -> [Int]
             -- аргументы функции
generateList initial elementsNumber multiplicity 
    -- Если начальное значение initial уже кратно числу multiplicity
    -- бесконечный список, с помощью take выбираются первые elementsNumber элементов
    | initial `mod` multiplicity == 0 = take elementsNumber [ initial, initial + multiplicity ..]
    | otherwise = do
        -- нужно найти ближайшее число   initialK - индекс ближайшего числа, кратного multiplicity   
        -- initial div multiplicity + 1= 15 div 7+ 1 = 3  [7 * 3, 7 * 4, 7 * 5, ...] = [21, 28, 35, ...]
