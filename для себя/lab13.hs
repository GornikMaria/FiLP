...
-- В Haskell, когда вы используете функцию hGetContents, данные считываются лениво, то есть содержимое файла не загружается в память до тех пор, 
-- пока вы не попытаетесь использовать его. Это может вызвать проблемы, если вы затем попытаетесь открыть тот же файл для записи, так как файл остается 
-- открытым для чтения, и попытка записи приведет к ошибке «resource busy» (ресурс занят).
